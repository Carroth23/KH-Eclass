...
//		}
		...
		try {
			if (cmd.equals("/toboard.board")) { // board눌렀을때 오는곳
				...
				if (currentPage > dao.getPageTotalCount()) {
					currentPage = dao.getPageTotalCount();
				}

//				1 -> 1 ~ 10
//				2 -> 11 ~ 20
//				3 페이지 에 있으면 21 ~ 30의 글이 보임
...
//				String contents = request.getParameter("contents");
				...
				if (result > 0) {
					...
					response.sendRedirect("/toboard.board?cpage=1"); // .board로 안보내고 페이지로 보내도 글이 올라오는지 확인해야됨 = 확인결과
																		// .jsp로 바로보내면 안올라옴
				}
